	...
	if( lyteDomObj ){
		...
		function valFormat( arg, format ){
			...
			if( isDef( copyFormat.hour ) ) {
				date.setHours( copyFormat.railway ? convertRailway( copyFormat.hour, copyFormat.meridian == 'PM' ) : ( copyFormat.meridian == 'PM' ? ( copyFormat.hour < 12 ? ( copyFormat.hour + 12 ) : copyFormat.hour ) : copyFormat.hour % 12 ) )
			}
